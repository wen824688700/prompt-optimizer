...
本实施计划将 Prompt Optimizer MVP 的设计转换为可执行的编码任务。任务按照依赖关系组织，每个任务都包含具体的实现步骤和需求引用。
...
- [ ] 5. Checkpoint - 后端服务测试
  - 确保所有后端服务和 API 端点通过测试，询问用户是否有问题

- [ ] 6. 前端页面组件实现
  - [ ] 6.1 实现 HomePage 组件
    - 创建输入框组件（最少 10 字符验证）
    - 创建模型选择下拉菜单（初期仅 DeepSeek）
    - 创建附件上传组件（文件类型和大小验证）
    - 创建优化按钮
    - 实现极简布局（Tailwind CSS）
    - _Requirements: 1.6, 2.1, 2.2, 2.4, 2.5, 2.6, 3.1_
  
  - [ ] 6.2 实现 ClarificationModal 组件
    - 创建框架选择组件（如果有多个候选）
    - 创建 5 个标准追问问题表单
    - 实现必填/可选字段验证
    - 实现提交和取消按钮
  ...
  - [ ] 6.3 实现 Workspace 组件
    - 创建 5:5 左右分栏布局
    - 左侧：可编辑输入框 + 重新生成按钮
    - 右侧：两个选项卡（Markdown 原文 + 版本记录）
    - 实现本地草稿自动保存
    - _Requirements: 1.7, 5.4, 5.5, 6.1_
  
  - [ ] 6.4 实现 MarkdownTab 组件
    - 展示 Markdown 格式提示词
    - 实现复制按钮（复制到剪贴板）
    - 实现修改按钮（覆盖到左侧编辑区）
    - 实现保存按钮（保存版本）
  ...
  - [ ] 6.6 实现 AccountPage 组件
    - 展示用户信息（头像、邮箱、账户类型）
    - 展示配额信息（已用/总计、重置时间）
    - 展示订阅信息（状态、下次扣费日期）
    - 实现升级到 Pro 按钮
    - 实现取消订阅按钮
    - _Requirements: 7.5, 8.7, 9.6_
...
- [ ] 7. 前端状态管理与 API 集成
  ...
  - [ ] 7.3 实现错误处理
    - 创建全局错误边界组件
    - 实现 API 错误提示（Toast/Modal）
    - 实现网络错误自动重试（3 次）
    - 实现配额不足提示
    - _Requirements: 10.1, 10.2, 10.3, 10.4, 10.5_

- [ ] 8. 用户认证集成
  - [ ] 8.1 配置 Supabase Auth
    - 初始化 Supabase client
...
- [ ] 10. Checkpoint - 前端功能测试
  - 确保所有前端组件和用户流程正常工作，询问用户是否有问题

- [ ] 11. 响应式设计与 UI 优化
  - [ ] 11.1 实现响应式布局
    - 桌面端：全宽布局
    - 移动端：单列布局
    - 工作台：5:5 分栏（桌面）/ 上下布局（移动）
...
- [ ] 13. 部署配置
  - [ ] 13.1 配置 GitHub Actions CI/CD
    - 创建 frontend-ci.yml（Lint + Test + Build）
    - 创建 backend-ci.yml（Lint + Test）
    - _Requirements: 11.1_
